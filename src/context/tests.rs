...
fn assert_modal_focus_trap_after_key(key: KeyCode) {
    let mut backend = TestBackend::new(40, 12);
    let mut outside_focused = false;
    let mut first_modal = false;
    let mut second_modal = false;
    let mut draw = |ui: &mut Context| {
        outside_focused = ui.button("outside").focused;
        let _ = ui.modal(|ui| {
            first_modal = ui.button("first").focused;
            second_modal = ui.button("second").focused;
        });
    };
    for _ in 0..3 {
        backend.render(&mut draw);
    }
    backend.render_with_events(EventBuilder::new().key_code(key).build(), 0, 2, &mut draw);
    backend.render(&mut draw);
    assert!(!outside_focused, "focus must remain inside the modal");
    assert!(!first_modal);
    assert!(second_modal);
}
...
fn modal_focus_trap_tabs_only_within_modal_scope() {
    assert_modal_focus_trap_after_key(KeyCode::Tab);
}

#[test]
fn modal_focus_trap_shift_tab_wraps_within_modal_scope() {
    assert_modal_focus_trap_after_key(KeyCode::BackTab);
}
