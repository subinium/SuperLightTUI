...
- **`refactor(context)`! — `use_memo` returns a `Memo<T>` handle** (#272) — `Context::use_memo` previously returned `&T`, a live immutable borrow of `&mut Context` that could not be held across later `ui.*` calls — the lone asymmetry in the hook family (`use_state*` already return owned `State<T>` index handles), and exactly the implicit "deref-and-copy-at-call-site" (`*ui.use_memo(...)`) rule LLM-generated code gets wrong. It now returns a `Memo<T>` index handle (re-exported at crate root) that holds **no** borrow of `Context`; read the value on demand with `.get(ui)` (`&T`) / `.copied(ui)` (`T: Copy`). `Memo<T>::get` panics with the same hook-index + expected-type message on a rules-of-hooks violation. The memo slot's internal storage changed from `(D, T)` to a `pub(crate) MemoSlot<T>` (type-erased deps) so the read path needs no `D` re-parameterization. The original `&T`-returning body is preserved verbatim as `use_memo_ref` (see Deprecated). **Migration:** `let x = *ui.use_memo(&d, f);` → `let x = ui.use_memo(&d, f).copied(ui);` (or `.get(ui)` for a reference); callers that cannot migrate immediately can switch to `ui.use_memo_ref(&d, f)` (warns). All in-repo callers migrated to `.copied(ui)`.
- **`feat(widgets-display)`! — `scrollbar()`, `separator()`, `separator_colored()` return real `Response`** (#241) — Resolves `Known v0.21 migration note #2` / `docs/ARCHITECTURE.md` M4 (`no () returns`). `separator()` / `separator_colored()` previously returned `&mut Self` from a legacy text-chain pattern, but the chained `.bold()` / `.fg()` mutators were a no-op (the cached separator string is already finalized), so the misleading chain is dropped in favor of a real `Response` routed through `Context::interaction()`. `scrollbar()` already returned `Response::none()` in v0.20 (#184); it is now routed through the track container's own interaction `Response` so the track hit-test rect is populated, and its receiver changes from `&ScrollState` to `&mut ScrollState` to reserve the click-to-jump / drag-to-scroll extension point (#249) without another breaking change. **Migration:** statement-form `separator` callers (`ui.separator();`) compile unchanged; any `.separator().bold()`-style chains must drop the no-op suffix (none existed in-crate); in-crate example / test / bench statements get `let _ =` to satisfy the `must_use` lint under `-D warnings`. `scrollbar` callers pass `&mut scroll` instead of `&scroll`.
- **`feat(widgets-display)`! — status-family widgets populate `Response` via `Context::interaction()`** (#242) — `badge`, `badge_colored`, `key_hint`, `stat`, `stat_colored`, `stat_trend`, `empty_state`, and `empty_state_action` previously returned `Response::none()` unconditionally — the signature was misleading (`if ui.badge("New").hovered { … }` compiled but never fired and `.on_hover(...)` tooltips could not attach). They now carry real `hovered` / `gained_focus` / `lost_focus` / `right_clicked` state for the widget's hit-test rect. The single-line widgets (`badge`, `badge_colored`, `key_hint`) reserve the slot via `Context::interaction()` before their text (matching the spinner / gauge / separator pattern); the column widgets (`stat*`, `empty_state`) return the `Response` the container layout already produces instead of discarding it. `empty_state_action` keeps `clicked` / `changed` tied to its action button while surfacing hover / right-click for the whole placeholder. **Migration:** statement-form callers (`ui.badge("New");`) compile unchanged; any caller asserting `response.hovered == false` now sees real state.
- **`refactor(widgets)`! — `ScrollState::progress_ratio() -> f64`; `progress() -> f32` deprecated** (#243) — `ScrollState::progress() -> f32` was the only `f32` ratio left after the v0.20 `f32 → f64` ratio unification (Gauge / LineGauge / SplitPaneState / `progress` / `progress_bar` / `animate_value` / GaugeResponse / SplitPaneResponse). `progress_ratio() -> f64` is now the canonical accessor and `progress() -> f32` is `#[deprecated(since = "0.21.0")]`, delegating to it. `gauge` / `progress_bar` already take `f64`, so call sites can drop any `as f64` cast. **Migration:** `scroll.progress()` → `scroll.progress_ratio()` (no in-repo callers existed). See Deprecated.
- **`refactor(context)`! — `use_state_named` hook family naming unified** (#240) — The `_named` family now follows the "no-suffix = init closure" convention so it matches `use_state(init)` and `use_state_keyed(id, init)` (per `docs/NAMING.md` "find an analogous method, match its shape"). `Context::use_state_named(id, init)` now takes an init closure (previously required `T: Default`); the `T: Default` shorthand moved to the new `Context::use_state_named_default(id)`, mirroring `use_state_keyed_default`. `use_state_keyed` / `use_state_keyed_default` already matched and are unchanged. **Migration:** `use_state_named::<T>("id")` → `use_state_named_default::<T>("id")`; `use_state_named_with("id", init)` → `use_state_named("id", init)`. See Deprecated.
- **`docs`! — reconcile `API_DESIGN.md` / `NAMING.md` with shipped surface; `code_block` becomes a consuming builder, `code_block_*` deprecated** (#271) — Resolves five doc-vs-code drifts so the canonical design docs compile against the actual crate: `alert` arg order corrected to the shipped `(message, level)`; Rule 1 rewritten to the shipped consuming-builder shape (`mut self -> Self`, Drop-render, `.show()`); `progress` doc corrected to the shipped `-> Response`; the `_colored` contradiction resolved by **blessing** the suffix as the canonical color-override variant (the immediate widgets that return `Response` have no chainable builder to host `.color()`), with `NAMING.md` gaining `_hd` / `_halfblock` / `_styled`. Code: a new `CodeBlock<'a>` consuming-builder (Drop-render, mirrors `Gauge`) — `ui.code_block(code).lang(l).numbered().show()` — so `code_block` now returns `CodeBlock<'_>` (renders on Drop, bare-statement callers unaffected), and the three `code_block_*` variants are `#[deprecated(since = "0.21.0")]` behavior-preserving aliases. `CodeBlock` is re-exported alongside `Gauge` / `Breadcrumb`. **Migration:** statement-form `ui.code_block(code);` compiles unchanged; `code_block_numbered` / `code_block_lang` / etc. → the `CodeBlock` builder. See Deprecated and Docs.

### Added

- **`feat(layout)` — `ContainerBuilder::wrap()` flex-wrap + `ContainerBuilder::basis(u32)` flex-basis** (#258) — SLT's analogue of ink/Yoga `flexWrap="wrap"` + `flexBasis`. `wrap()` makes a `row()` flow children that overflow the available width onto subsequent lines (multi-line row), stacking lines on the cross axis — the immediate-mode primitive for tag clouds, chip lists, wrapping toolbars, and responsive card grids that reflow on resize without per-frame breakpoint math. Within-line spacing uses `gap`/`col_gap`; between-line spacing uses `row_gap` (else `gap`). A child wider than the full width occupies its own line (clipped, as a single-line row clips), and a wrapping row reports the correct multi-line `min_height` so a parent `col()` reserves enough rows. `wrap()` is row-only (a documented no-op on `col()`). `basis(u32)` sets the flex-basis — the initial main-axis size that `grow` grows from and `shrink` (#161) shrinks from — defaulting to the child's min-width (current behavior) when unset. Both are opt-in and non-breaking. Threaded via new `Command::WrapMarker(i32)` (carrying the cross-axis gap) / `Command::BasisMarker(u32)` scalar markers (mirroring `ShrinkMarker` #161), so the `Command` enum stays `<= 128` bytes and `WidthSpec` / `HeightSpec` / `Constraints` are untouched (their 24-byte guard holds). `layout_row` now extracts a shared `layout_row_line` per-line body; the wrap pass partitions children greedily and lays each line out with it. Preserves the signed `gap_overlap` gap math (#222). `_ASSERT_LAYOUT_NODE_SIZE` bumped 320 → 328 for the two new 4-byte scalar fields (`cross_gap: i32`, `flex_basis_raw: u32`).
- **`feat(layout)` — `ContainerBuilder::gap_overlap(u32)` signed inter-child gap** (#222) — SLT's analogue of ratatui `Layout::spacing(-1)`. `gap_overlap(1)` makes adjacent children overlap by one cell on the main axis so two adjacent bordered panels collapse their duplicate border edge into a shared column (row layout) or row (column layout). The internal gap pipeline (`LayoutNode::gap`, `ContainerConfig::gap`, `BeginContainerArgs` / `BeginScrollableArgs::gap`, `ContainerBuilder::gap`) is now `i32`; `layout_row` / `layout_column` / `justify_offsets` and the intrinsic-size kernels use `i64` intermediates with `saturating_add_signed`, so an overlap larger than a child's extent saturates at 0 — no panic, no wrap. The public `gap(u32)` / `row_gap` / `col_gap` / `ContainerStyle` APIs are unchanged (source-compatible); only `gap_overlap` can store a negative value. Rendering of the shared cell is last-write-wins (no automatic `┼` junction merge yet — tracked as a follow-up), so overlapping panels should use compatible border styles. `_ASSERT_LAYOUT_NODE_SIZE` is unaffected (`i32` is the same 4 bytes as `u32`).
- **`feat(context)` — in-frame async `ctx.spawn` / `ctx.poll` API (feature `async`)** (#234) — `Context::spawn<T>(future) -> TaskHandle<T>` launches a future onto the ambient Tokio runtime from inside the frame closure; `Context::poll(&handle)` retrieves the result on a later frame (`Some` exactly once, then `None`). Closes the ergonomics gap versus `run_async`'s external mpsc channel for the common "click → spawn fetch → show result next frame" flow (bubbletea `tea.Cmd` inspiration). Dropping a `TaskHandle` cancels the in-flight task (`JoinHandle::abort`), drained every frame via `AsyncTasks::maintain()` in the frame-kernel round-trip so cancellation fires even on a frame that calls neither `spawn` nor `poll`. The registry round-trips through `Context` exactly like the scheduler table.
- **`feat(context)` — zero-dep frame-clock scheduler** (#248) — A wall-clock, async-free scheduler built on the existing per-frame state round-trip (mirroring `named_states`). New `Context` methods: `schedule(id, dur) -> bool` (one-shot, fires once at/after the deadline), `every(id, dur) -> u32` (recurring, returns whole intervals since last frame, never dropping or double-counting across stalled frames), `debounce(id, dur, dirty) -> bool` (typeahead primitive; fires once after a quiet window, re-armed on every dirty frame), `exclusive(group, id) -> bool` (most-recent claim wins, superseded ids stay cancelled — Textual `@work(exclusive=True)` semantics), `cancel(id)`, and `elapsed(id) -> Option<Duration>`. Deadlines use a single `std::time::Instant` sampled at frame start (`frame_instant`), not `diagnostics.tick`, because `run_frame_kernel` never advances the tick. The `SchedulerState` timer table is moved into `Context` at frame start and back at frame end, where untouched slots are GC'd so abandoned timers cannot leak. Works under `--no-default-features` and without the `async` feature; no new dependencies. Complements #234 (which owns off-thread work); this only defers work to a future frame on the existing loop.
- **`feat(theme)` — external TOML theme files + serde hot-reload watcher** (#267) — A file-backed theming pipeline behind the existing `serde` feature plus a new optional `theme-watch` feature for live hot-reload. `Color::from_hex` / `to_hex` and a custom symmetric serde impl serialize/deserialize colors as human-friendly tokens (`"#rrggbb"` / `"#rgb"`, named, `"indexed:N"`) that round-trip through TOML. `WidgetTheme` gains serde derives; `WidgetTheme` / `WidgetColors` / `Theme` get struct-level `serde(default)` so partial documents fill from defaults. New `ThemeFile { theme, widgets }`, `ThemeLoadError`, and convenience `Theme::from_toml_str` / `Theme::load` in `src/style/theme_io.rs`. `ThemeWatcher` (feature `theme-watch`, via `notify`) offers a non-blocking `poll()` that returns the latest parsed theme on change and keeps the last good theme on a parse error (no panic, logs to stderr). New `examples/theme_hot_reload.rs`. Optional deps `toml` (under `serde`) and `notify` (under `theme-watch`) stay out of the default and WASM builds. Purely additive; no existing fields, signatures, or derives changed.
- **`feat(style)` — BLINK / OVERLINE modifiers, underline color/style, theme-aware syntax palette** (#266) — Closes three text-styling gaps versus ratatui / bubbletea / ink. (1) `Modifiers` gains `BLINK` (`1<<6`) and `OVERLINE` (`1<<7`) using the two spare `u8` bits, with `Style::blink()` / `overline()` builders; `apply_style_delta` / `apply_style` emit `SlowBlink` / `NoBlink` and `OverLined` / `NotOverLined`. (2) `Style` gains `underline_color: Option<Color>` and `underline_style: UnderlineStyle` (`Straight` / `Double` / `Curly` / `Dotted` / `Dashed`, default `Straight`); both builders imply the `UNDERLINE` bit and emit the raw escapes crossterm cannot express — `CSI 4:Nm` for non-straight styles and `SGR 58` / `SGR 59` for underline color. (3) `Theme` gains `syntax: SyntaxPalette` (9 token-category colors); every preset supplies a palette and `ThemeBuilder` gets a `syntax()` setter, so `highlight_name_to_style` now resolves every colorful token through `theme.syntax.*` with no hardcoded One Dark RGB literals remaining (`dark()` / `one_dark()` output preserved byte-for-byte). `HIDDEN` is deferred (the `Modifiers` `u8` has exactly two free bits, both consumed here; adding it requires widening to `u16`, a later breaking change). `Cell` still fits the 64B cache-line budget. New `SyntaxPalette` / `UnderlineStyle` re-exported from the crate root. Non-breaking: `Style` / `Theme` extended with defaulted fields, `Modifiers` adds constants only.
- **`feat(buffer)` — RTL/bidi text reordering to visual order via UAX #9** (#260) — Every string passed to `Buffer::set_string` / `set_string_linked` was stored in logical order and painted left-to-right by column, so RTL scripts (Hebrew, Arabic, Farsi, Syriac, Thaana) rendered mirrored and mixed LTR+RTL runs were scrambled — terminals in cell-addressed mode do not apply the Unicode Bidirectional Algorithm themselves. A new `bidi` module helper is added at the single chokepoint `set_string_inner`: `needs_bidi_reorder` is a cheap conservative character-class pre-scan that lets pure-LTR input skip the reorder entirely (no allocation, no `unicode-bidi` call — byte-identical to before), and `reorder_line_visual` runs UAX #9 only on lines that actually contain RTL / explicit-bidi codepoints. Existing width, clip, zero-width-attachment, wide-char trailing-blank, control-char sanitization, and OSC 8 hyperlink logic are unchanged and apply to the reordered glyph stream. Gated behind a new default-on `bidi` Cargo feature (`unicode-bidi` 0.3, optional); under `--no-default-features` the fast path is the only path (identical to v0.20.1). Render-only: input state stays logical; caret/selection mapping and Arabic shaping are out of scope.
- **`feat(event)` — opt-in Kitty `REPORT_ALL_KEYS` + `KeyCode::Modifier` variant** (#261) — SLT could not deliver modifier-only key events (a bare Ctrl / Shift / Alt / Super press or release): the Kitty flags pushed omitted `REPORT_ALL_KEYS_AS_ESCAPE_CODES`, so a spec-compliant terminal never emitted them, and `from_crossterm` had no `KeyCode::Modifier` variant to receive them. Adds a public `ModifierKey` enum mirroring crossterm's `ModifierKeyCode` with SLT naming, a `KeyCode::Modifier(ModifierKey)` variant, and a `from_crossterm` arm mapping all 14 variants. A new opt-in `RunConfig.report_all_keys` bool (default `false`, no effect without `kitty_keyboard`) is threaded through `Terminal` / `InlineTerminal::new` → `TerminalSessionGuard` → `write_session_enter` via a pure `kitty_flags` helper. `EventBuilder` gains `key_modifier()` for headless tests.
- **`feat(keymap)` — cross-frame `key_chord` sequence matching; `key_seq` deprecated** (#262) — `key_seq` was advertised as multi-key sequence detection (vi `gg`, leader keys) but only matched when every key arrived in one poll batch, making chords unreachable at human typing speed. A persistent per-context `ChordState` buffer now round-trips through `FrameState` (same `std::mem::take` out/in pattern as `keyed_states`); the new `key_chord` / `key_chord_timeout` buffer partial input across frames, fire exactly once on completion, consume the completing key so downstream widgets do not double-handle it, reset on a mismatching key (longest-suffix overlap, vi semantics), and expire stale prefixes via the tick clock used by notifications/animation (default `DEFAULT_CHORD_TIMEOUT_TICKS = 60`, ~1s at 60Hz). Leader notation (`<space>` / `<leader>` tokens, or a literal space) is supported; modifier chords remain out of scope. `key_seq` is kept as a `#[deprecated(since = "0.21.0")]` thin wrapper delegating to `key_chord`, so existing callers compile and silently gain correct cross-frame behavior. See Deprecated.
- **`feat(widgets)` — interactive color picker widget** (#257) — Adds a palette / swatch-grid + hex-entry color picker over the existing `Color` model, closing the completeness gap for theming, settings, and design-tool UIs (there was no runtime way to choose a `Color`). `ColorPickerState` + `PickerMode` ship with `new(colors)`, `tailwind()` (22 c500 shades), `columns()`, and a `selected()` accessor that prefers a parsed `#RRGGBB` / `#RGB` hex value. `Context::color_picker` / `color_picker_colored`: arrow keys / hjkl move a clamped 2D cursor (no wrap), Tab toggles palette / hex, hex keystrokes route to an embedded `TextInputState` with validation, Enter / Space confirm, and left-clicks hit-test swatch cells (all handled keys / clicks consumed). Swatches emit a full-RGB background and a `contrast_fg` marker; the terminal flush layer downsamples per `ColorDepth`, and each `Rgb` swatch carries a `#RRGGBB` label so the picker stays legible under `NoColor`. New `WidgetTheme.color_picker` slot + const builder; `ColorPickerState` / `PickerMode` re-exported from the crate root; new `color_picker` example.
- **`feat(widgets)` — standalone `PaginatorState` + `paginator()` widget** (#256) — Pagination math and key/click navigation were welded onto `TableState`, so there was no composable way to page over arbitrary item counts (wizards, slide decks, carousels). Adds a standalone `PaginatorState` owning a page index over a generic item count, plus a `paginator()` / `paginator_colored()` interactive widget with `Dots` (default) and `Arabic` styles (`PaginatorStyle`). Follows the `tabs()` pattern: register focus, consume Left / h / PageUp (prev) and Right / l / PageDown (next), click a dot to jump, click counter halves to step. Dots auto-fall back to Arabic past 12 pages to avoid overflow. `Response.changed` iff the page moved; no-op-safe when `total_items == 0`. `TableState` is left untouched (sharing the page math is deferred). Re-exported from `lib.rs`.
- **`feat(widgets-input)` — `number_input` stepper widget** (#255) — Adds a stateful numeric stepper field as a sibling to the bar-style slider. `NumberInputState` holds value / min / max / step plus an integer-mode flag, an in-progress edit buffer, and a `parse_error`. `Context::number_input` renders a `▾ value ▴` field that adjusts via Up/k (+step) and Down/j (−step) clamped to `[min, max]`, via the scroll wheel over the field rect (`prev_hit_map` pattern), and via direct typing (digits, one `.` in float mode, leading `-` when `min < 0`) — committing on Enter, discarding on Esc, editing on Backspace. Invalid input sets `parse_error` and leaves the committed value untouched; integer mode rounds to whole numbers, float mode formats via `format_compact_number`. Consumes all handled key/scroll events and themes via `theme.primary` / `text_dim` / `error`. `NumberInputState` re-exported from `lib.rs` and the `context.rs` widgets facade.
- **`feat(widgets-interactive)` — variable-height `virtual_list` (per-item height)** (#253) — Adds a row-accurate variant of `virtual_list` so chat / feed bubbles of differing heights virtualize correctly while preserving the existing fixed-height fast path byte-for-byte. `ListState` gains an optional per-item height model: caller-provided `item_heights` (clamped to `>= 1`) plus a lazily-rebuilt prefix sum (`row_prefix`) gated behind a dirty flag, and a `viewport_row_offset` that tracks the cumulative top-row offset while `viewport_offset` keeps its "top item index" meaning. New `with_item_heights` / `set_item_heights` / `clear_item_heights`. `Context::virtual_list_variable` computes the visible item range so the rendered items sum to at most `visible_height` rows; PageUp / PageDown advance by the item count that fills a viewport of rows, not a fixed item count. An item taller than the viewport renders from its top and is never skipped. Rendering stays O(visible): prefix-sum lookups are O(log n) via binary search, rebuild is O(n) behind the dirty flag. With no per-item heights set, output is identical to the existing `virtual_list` (both delegate to a shared `virtual_list_impl`). Purely additive, not breaking.
- **`feat(widgets-interactive)` — table per-column renderer, column constraints, multi-row selection** (#251) — Extends `TableState` and `Context::table` with three structural capabilities while keeping the existing string-grid API byte-identical for all-`Auto`, single-select callers. (1) A `TableColumn` width enum (`Auto` / `Fixed` / `Min` / `Max` / `Percent`) settable per column via `TableState::column_widths_spec`; `resolve_column_widths` overlays the spec onto the content-derived widths each frame (`Percent` against the live area width), clamping/truncating cells with an ellipsis to fit `Fixed` / `Max`. (2) Multi-row selection: a `multi_selected` HashSet + `selection_anchor` with `toggle_row` / `select_range` / `select_single` / `clear_selection` helpers and `prune_selection` wired into `rebuild_view` so stale view indices never leak on `set_rows` / `set_filter` / sort. Space toggles the focused row, Ctrl+Space toggles without clearing, Shift+Up/Down extends a contiguous range; plain click replaces with one row, Shift+click ranges from the anchor, Ctrl+click toggles. (3) `Context::table_with(state, |row, col, raw| -> (String, Style))`: a per-frame per-column renderer (no stored closure, so `TableState` keeps deriving `Clone`). `Response.changed` now also fires on multi-selection changes; selected members render with a dimmed background distinct from the brighter cursor row. `TableColumn` re-exported from `lib.rs`.
- **`feat(widgets-input)` — per-field form validators with triggers, built-ins, aggregate errors** (#252) — Moves form validation from a positional, fn-pointer slice to per-field, state-capturing closures co-located with each field. Adds a `Validator` newtype wrapping `Box<dyn Fn(&str) -> Result<(), String>>` (so validators can close over state — impossible with the old `fn` pointer), a `ValidateTrigger` (`OnChange` / `OnBlur` / `Manual`, default `OnBlur`), and a chainable `FormField::validate()` builder with `.on_change()` / `.on_blur()` / `.manual()` and `run_validators()`. `Context::form_field` now runs a field's validators automatically per its trigger (blur is derived from a focus edge tracked on the field via `observe_focus`, so the `text_input` public API is unchanged). A `validators` built-in module ships `required`, `min_len`, `max_len`, `email`, `range_i64`, `range_f64`, `one_of`, and a handwritten glob-style `regex` (no regex / email crate dependency). `FormState` gains `is_valid()`, `errors()`, `validate_all()`, and `validate_with()` (cross-field rules); the positional `validate(&[..])` is retained as a `#[deprecated(since = "0.21.0")]` shim. Async validation behind the `async` feature: `validate_async` spawns on the Tokio runtime and `form_field` polls a oneshot each frame. `Validator` / `ValidateTrigger` / `validators` / `AsyncValidation` (async-gated) re-exported from the crate root. See Deprecated.
- **`feat(calendar)` — range-select mode + optional time-of-day** (#254) — Extends `CalendarState` with an opt-in range mode (anchor + extent stored as absolute `CalDate`s so a range can span month/year boundaries) and an optional hour/minute row; single-date mode stays the default and renders byte-for-byte unchanged. New public types `CalDate` / `CalendarSelect`; new methods `with_range`, `with_time`, `mode`, `selected_range`, `selected_time` (all additive). Keyboard: Shift+Left/Right/Up/Down and Shift+H/L extend the range from the anchor by the same ±1/±7 day deltas; Enter/Space sets the anchor, Shift+Enter/Space sets the extent (plain h/l and `[`/`]` month bindings from v0.20 preserved). Mouse: plain left-click anchors, Shift+left-click sets the extent (new `EventBuilder::click_with` carries modifiers). Render: the interior range band uses `theme.surface`, endpoints use `selected_bg/fg`, cursor highlight retained; the time row renders `HH:MM` only when enabled. `Response.changed` is the OR of selection/range/time deltas.
- **`feat(widgets-interactive)` — type-to-filter on the `select` dropdown** (#250) — An open `select` with more than a handful of options was impractical with cursor-only navigation. Typing now filters the option list (fuzzy subsequence match, reusing `CommandPaletteState::fuzzy_score`), arrow keys navigate the filtered subset, Backspace edits the query, and Esc clears a non-empty query before closing; Enter selects the cursor's filtered item. `SelectState` gains a public `filter` field (callers can pre-fill or inspect the live query) and a `filtered_indices()` helper. The cursor now indexes the filtered subset; printable keys (including `j` / `k` / space) type into the filter, so arrow keys are the sole navigation while open. An empty query preserves the prior rendering byte-for-byte.
- **`feat(terminal)` — runtime capability probe + automatic blitter ladder** (#264) — SLT now negotiates with the terminal at session enter instead of guessing from `TERM` / `TERM_PROGRAM` allowlists. A one-shot DA1 (`CSI c`) / DA2 (`CSI > c`) / XTGETTCAP + Kitty-graphics probe (≤150ms, reusing the existing OSC round-trip infra, cached in a `OnceLock`) fills a read-only `Capabilities` snapshot (`truecolor`, `sixel`, `kitty_graphics`, `kitty_keyboard`, `sync_output`, and a `BlitterSupport { half, quad, sextant }` set), exposed via `Context::capabilities() -> &Capabilities` and re-exported from the crate root alongside `Blitter` / `BlitterSupport`. DA1 attribute `4` confirms sixel; XTGETTCAP `Tc` (or `COLORTERM`) confirms truecolor; Kitty graphics is confirmed via the APC `_G i=31` ack. Image rendering routes through `Capabilities::best_blitter()` — an automatic ladder (Kitty graphics > Sixel > sextant > half-block) where the first supported rung wins and **app code never selects a protocol**. `sixel_image()` now consults the probed `sixel` flag first, falling back to the env allowlist only when the probe is silent. Companion fix: WezTerm and Ghostty are added to the `terminal_supports_sixel()` env-fallback allowlist (via `TERM_PROGRAM`) so both render images instead of `[sixel unsupported]`. On a headless backend (`TestBackend` / piped stdout) the probe is skipped and every field defaults conservatively. Non-breaking: all additive; `SLT_FORCE_SIXEL` still forces sixel. Out of scope (tracked separately): a real sextant renderer (the ladder slot falls through to half-block) and animated image protocols.
- **`feat(terminal)` — iTerm2 OSC 1337 inline-image blitter + sprixel damage matrix** (#265) — Adds the fourth image path alongside Kitty / Sixel / half-block: `Context::iterm_image(data, cols, rows)` and `Context::iterm_image_fit(data, cols)` emit a spec-correct OSC 1337 `File=inline=1;size=..;width=..;height=..;preserveAspectRatio=..:<base64>\x07` sequence. Unlike Kitty/Sixel (raw RGBA), the iTerm2 path takes **encoded image-file bytes** (PNG/JPEG/GIF) and lets the terminal decode and scale — the only pixel-accurate path on Tabby, older iTerm2 builds, and WezTerm's iTerm2-compat mode. Detection integrates into the #264 capability ladder: a new `Capabilities.iterm2` flag is filled by env identity (`TERM_PROGRAM` in {iTerm.app, WezTerm, Tabby, mintty}, or `SLT_FORCE_ITERM=1`) and `Blitter::Iterm2` slots between Sixel and Sextant. On an unsupported terminal both methods reserve the `cols`×`rows` box and draw `[iterm2 unsupported]`, mirroring the Sixel fallback; `#[cfg(not(feature = "crossterm"))]` stubs keep `--no-default-features` builds compiling. Second: a **sprixel damage matrix** (notcurses-style `SprixelCell::{Opaque, Mixed, Transparent, Annihilated}` per owned cell) replaces the all-or-nothing `flush_raw_sequences` guard for the non-Kitty paths. Sixel now routes through `Buffer::sprixel_place` (same observable output, fewer re-blits); a pixel graphic is re-emitted **only** when its `(x, y, content_hash)` changed or a text write annihilates a covered cell — a text edit in a transparent/uncovered region of a Sixel now emits **zero** passthrough bytes. Kitty keeps its existing `KittyImageManager` lifecycle unchanged. Non-breaking and purely additive; `iterm_image*` are new, `sixel_image` / `kitty_image*` / `image` signatures and behavior are unchanged.
- **`feat(widgets-display)` — `scrollbar()` click-to-jump + thumb drag** (#249) — Fills in the behavior behind #241's `&mut ScrollState -> Response` signature: the bar is now a real input surface, mirroring `split_pane`'s drag handle. A left mouse-down inside the track but outside the thumb jumps `state.offset` so the clicked row maps proportionally to the content (top cell → offset 0, bottom cell → `max_offset`); a mouse-down on the thumb sets `ScrollState::dragging`, and subsequent drags scroll proportionally to the cursor's y within the track even when the cursor leaves the track on the x-axis, until mouse-up clears `dragging`. The handler hit-tests the previous-frame track rect from `prev_hit_map` and consumes only the mouse events it acts on, so wheel scrolling over a sibling `scrollable()` keeps working and is never double-counted; offset moves clamp via the existing `scroll_down` max-offset math, and a moving frame reports `Response.changed = true`. Modal suppression matches `mouse_down`; with no overflow the bar renders nothing, consumes nothing, and leaves drag state untouched. `ScrollState` gains a `pub dragging: bool` field (default `false`) and a `pub fn set_offset(&mut self, offset: usize)` clamping setter — both additive and non-breaking. Vertical-only; horizontal click-jump and page-on-track-click stay out of scope.
- **`feat(lib)` — devtools inspector (focused-widget style panel + focus chain)** (#268) — Extends the F12 layout debugger (#201) with an in-process devtools inspector toggled independently via Ctrl+F12. When active and a widget has focus, a resolved-style panel shows the focused widget's index, name, layout rect, fg/bg color, padding, and constraints; a focus-chain panel lists every focusable in order with a `>` cursor on the current focus and the registered name for any named entry. When nothing is focusable a single notice line is drawn. This reuses the already-built layout tree (one focused-node DFS) and the focus snapshot threaded in from `FrameState` — no new per-frame traversal or collection. New `inspector()` / `set_inspector()` public API; `DiagnosticsState.inspector_mode` defaults to `false` so existing apps render identically. Not breaking. F12, Shift+F12, `set_debug`, and the status-bar format are untouched.
- **`feat(context)` — opt-in scoped cached region `ContainerBuilder::cached(version_key, f)`** (#273) — An *author-controlled* stability gate for the LLM token-streaming workload, where "append one token, re-render the whole frame" re-walks the entire pipeline (closure → `build_tree` → `compute` → `collect_all` → `render`) every token even for large static chrome (#171's row-hash only short-circuits the downstream flush stage). `cached(key, f)` lets the author declare a subtree stable via a `u64` key they already own (e.g. a hash of the non-streaming inputs); when the key is unchanged from the previous frame at that call site it is classified as a *cache hit*, otherwise a *miss* (key change / new call site / first frame / resize). **This is not reactive binding** (permanently rejected, `docs/COMPETITIVE_ANALYSIS.md` P2-4 / `docs/DESIGN_PRINCIPLES.md` R2): `f` runs **every frame** exactly like `.col(f)`, so output is **byte-for-byte identical** to an uncached container in all cases and there is zero behavior change when unused. The current implementation lands the *safe, reversible* half — a measured, author-keyed gate plus hit/miss diagnostics (`Context::region_cache_hits()` / `region_cache_misses()`) — and **deliberately does not** skip `f` on a hit, because eliding the body would require splicing recorded commands and replaying focus / hit-map / scroll / raw-draw feedback, which risks reintroducing the retained tree R2 forbids (out of scope; tracked as a follow-up). New `StreamingTextState::version()` / `StreamingMarkdownState::version()` (monotonic, bumped on `push` / `start` / `clear`) expose the stream delta so callers key the *surrounding chrome* off it ("cache the chrome, not the stream"). Phase-0 streaming baseline added as `bench_streaming_append_chat` (`benches/benchmarks.rs`). Per-call-site keys round-trip through `FrameState::region_versions` (same alloc-reuse + recycle discipline as `commands_buf`) and are invalidated on resize. Non-breaking; all additive. `Command` enum and `LayoutNode` size guards untouched (no new variants/fields).
- **`feat(test-utils)` — `PtyBackend` + `PtyFrame` end-to-end escape-byte harness** (#274) — Behind the dev-only `pty-test` feature (off by default), `PtyBackend` drives the *real* `Terminal` flush pipeline into an in-process `Vec<u8>` sink, so the actual escape/image-protocol bytes that ship to a terminal — SGR runs, OSC 8 hyperlinks, Sixel (`\x1bPq`), Kitty graphics (`\x1b_Ga=`), and color-depth-downsampled SGR — are asserted as whole frames. A private `Sink` indirection in `Terminal` lets `Terminal::with_sink` target the in-process buffer; the production `Terminal::new` / `run()` path is byte-identical. Assertions: `assert_emits`, `assert_not_emits`, `last_raw`, `frames_raw`, `with_color_depth`. End-to-end byte assertions in `tests/pty_ansi.rs` cover a styled-text SGR run, a Sixel envelope, a Kitty APC, an OSC 8 hyperlink, and truecolor-vs-256 SGR divergence (the styled-Red case pins crossterm's real `38;5;1` emission, not legacy `31`). This is the byte/protocol regression tier the buffer-only `TestBackend` and plain-text `insta` snapshots deliberately cannot reach. No real TTY required — the in-process `Vec<u8>` sink reproduces on `ubuntu-latest`. See CI.

### Fixed

- **`fix(layout)` — honor embedded `\n` in `wrap_lines` / `wrap_segments`** (#246) — `text(...).wrap()` collapsed pre-formatted multiline strings into a single line: `\n` has display width 0 and was glued into the current word token, so it never triggered a break. This broke every path that renders text carrying hard line breaks — LLM / streaming output, file contents, and error / stack-trace dumps (exactly SLT's AI-native target). Hard breaks are now resolved before soft wrapping: split on `\n` (CRLF normalized) into paragraphs, then word-wrap each independently. `wrap_lines` uses `split('\n')` (not `str::lines()`) so a trailing `\n` yields a trailing empty line; `wrap_segments` factors its kernel into `wrap_segments_paragraph` and keeps a no-newline fast path, so output is byte-identical for the common no-break case. A `\n` / `\r` never reaches a cell.
- **`fix(layout)` — `scroll_row()` honored direction; horizontal scrolling added** (#247) — `Context::scroll_row` advertised horizontal scrolling but silently built a `Direction::Column` scrollable: `finish()` discarded the caller's `.row()` / `.col()` direction, so the scroll engine only ever translated and clipped on the y-axis. The fix threads the caller's direction and an x-offset through the pipeline (`BeginScrollableArgs` gains `direction` + `scroll_offset_x`; `LayoutNode` gains `scroll_offset_x` + `content_width`; `BeginScrollable` honors `direction` instead of hardcoding `Column`) and adds a full x-axis mirror of the vertical scroll path: a `Direction::Row` flexbox arm that measures natural width and lays out into an over-wide virtual area on overflow, x-clipping in `visible_area`, a `set_string_clipped_x` writer that trims leading off-screen columns, x-translation of borders / scroll children, and left/right scroll indicators. `collect_all` shifts every collected rect by `x_offset` exactly as it does `y_offset`. `ScrollState` gains `offset_x` / `content_width` / `viewport_width` plus `scroll_left` / `scroll_right` / `can_scroll_left` / `can_scroll_right` / `progress_x` / `set_bounds_x`; `auto_scroll_nested` handles horizontal wheel + shift+wheel. Not breaking: all new types are `pub(crate)`; `ScrollState` and `scroll_row` signatures are unchanged. (`_ASSERT_LAYOUT_NODE_SIZE` 328 → 336 for two scalar `u32`s.)
- **`fix(layout)` — grapheme-cluster segmentation for wrap / truncate / cursor** (#259) — SLT segmented text by Unicode scalar everywhere a string is wrapped, truncated, or a cursor moves, so a user-perceived character spanning multiple scalars (ZWJ flag 🇰🇷, family emoji 👨‍👩‍👧‍👦, Devanagari क्षि, Thai กำ) was sliced mid-cluster on a wrap / truncate boundary or under `split_long_word`, and textarea / text_input cursors stepped into the middle of a cluster. Every segmentation site now routes through extended grapheme clusters (`unicode-segmentation` `graphemes(true)`), measuring width on the whole cluster via `UnicodeWidthStr`. The recent paragraph-split-on-`\n` behavior (#246) is preserved — iteration is now per-cluster *within* each paragraph; the no-newline fast path is unchanged. `truncate_with_ellipsis` stops on cluster boundaries (an overflowing cluster is dropped whole); new `grapheme_count` / `byte_index_for_grapheme` / `cluster_width` helpers; textarea / text_input Left/Right/Home/End/Backspace/Delete and column counts step by cluster. Adds `unicode-segmentation = "1"` as an unconditional core dep (no_std-friendly / WASM-safe). ASCII / Latin-1 behavior is byte-identical (cluster index == scalar index for ASCII).
- **`fix(terminal)` — handle SIGTSTP / SIGCONT job-control suspend/resume** (#263) — On Unix, Ctrl+Z (SIGTSTP) stops the process in kernel space before any `Drop` glue or panic hook can run, leaving the terminal in raw mode on the alternate screen with the cursor hidden, with no re-entry on SIGCONT (`fg`) — so the terminal stayed broken until exit / `reset`. A signal-hook background thread is now installed per run loop (all four modes: `run_with`, `run_inline_with`, `run_static_with`, `run_async_loop`) that, on SIGTSTP, runs the full session teardown (disable raw mode, leave alt screen for fullscreen, show cursor, disable bracketed paste / focus / mouse / kitty) then re-raises the default-disposition SIGTSTP to genuinely stop; on SIGCONT it re-enters the same session from a `RunConfig` snapshot and flags `NEEDS_FULL_REDRAW`, consumed at the top of each loop iteration to force a clear + repaint. New `RunConfig::handle_suspend` (default `true`, opt-out builder; `#[non_exhaustive]` keeps it backward-compatible). Unix-only, gated `#[cfg(all(feature = "crossterm", unix))]` — a no-op on Windows / WASM / non-crossterm. Uses only signal-hook's safe API, preserving `#![forbid(unsafe_code)]`; `signal-hook` is a unix-gated optional dep pulled by the `crossterm` feature (already vendored transitively, so no new lock entries).

### Perf

- **`perf(flush)` — eliminate per-frame allocations in the hot flush loop** (#269) — Hoists the run-coalescing scratch and swaps the per-frame heap allocations in the flush path for reused / borrowed state. None was catastrophic alone, but they compound on style-dense full redraws and image-heavy Kitty frames — the two workloads where SLT most wants to beat ratatui / bubbletea / ink. Output is byte-identical; both fast paths (per-row hash skip #171, Kitty placement diff #206) are preserved. `run_buf` is hoisted to a reused `String` field on `Terminal` + `InlineTerminal` (cleared-not-dropped each frame, capacity persists) and threaded `&mut` into `flush_buffer_diff`; OSC 8 hyperlink opens emit via three borrowed `Print()`s instead of a throwaway `format!`ed `String`; the Kitty flush replaces its two per-frame `HashSet`s with reused `SmallVec` dedup scratch (linear `contains` + sorted `binary_search`, faster at the 0–8 image counts this path sees, no SipHash); `compress_rgba` returns `Cow<[u8]>` so the no-compression path borrows the RGBA slice instead of cloning; and per-row hashing swaps `DefaultHasher` (SipHash) for a hand-rolled, dependency-free FNV-1a (row/image equality is not a security boundary, so the crypto-strength hash is needless per-frame tax — a deliberate deviation from the issue's `ahash` suggestion to avoid a new dependency and trip no `cargo deny` / `audit` gate).
- **`perf(docs)` — committed criterion baselines + ratatui head-to-head; wide and animation benches** (#270) — Replaces the unmeasured `TODO: measure` target table in `docs/PERFORMANCE.md` with real `cargo bench` medians and backs the speed claims with committed numbers. `benches/benchmarks.rs` gains a `full_render_dims` group (80×24, 120×40, 300×100 sharing one `render_dashboard` body), an `animation/churn_200x60` bench (content / progress / sparkline change every iter, forcing a non-empty diff each frame), `headtohead_200x60/{slt,ratatui}` rendering the same dashboard into each framework's in-memory backend (no real I/O), and `flush/full_redraw_300x100` + `flush/sparse_change_300x100`. `docs/PERFORMANCE.md` adds a "Measured baselines" table (Apple M3 Pro, macOS 26.4, rustc 1.95.0) and a "Head-to-head vs ratatui" subsection, and fixes stale pipeline line refs. `ratatui = "0.29"` is added to `[dev-dependencies]` only — it never enters the published `superlighttui` dependency tree. No runtime / library code changes (measurement + docs honesty only).
...
- **`deprecated(widgets)` — `ScrollState::progress`** (#243) — `progress() -> f32` is `#[deprecated(since = "0.21.0")]`, delegating to `progress_ratio() -> f64`. Migrate to `progress_ratio()` to drop any `as f64` cast at gauge / progress-bar call sites.
- **`deprecated(keymap)` — `Context::key_seq`** (#262) — Now a `#[deprecated(since = "0.21.0")]` thin wrapper delegating to `key_chord`. Existing callers compile and silently gain correct cross-frame sequence behavior; migrate to `key_chord` / `key_chord_timeout`.
- **`deprecated(widgets-display)` — `code_block_numbered` / `code_block_lang` / `code_block_numbered_lang`** (#271) — The three `code_block_*` variants are `#[deprecated(since = "0.21.0")]` behavior-preserving aliases now that `code_block` returns a `CodeBlock<'_>` consuming-builder. Migrate to `ui.code_block(code).lang(l).numbered().show()`.
- **`deprecated(widgets-input)` — `FormState::validate(&[..])`** (#252) — The positional, fn-pointer `validate(&[..])` is retained as a `#[deprecated(since = "0.21.0")]` shim with identical behavior. Migrate to per-field `FormField::validate()` builders plus `FormState::validate_all()` / `validate_with()`.

### CI

- **`ci(gallery)` — VHS gallery gate + tape coverage parity** (#274) — A new `gallery` job renders every `.tape` in the repo through VHS and asserts each produces its declared `Output` asset at a non-trivial byte size, uploading the generated assets as a workflow artifact (no auto-commit); `*.tape` is now tracked. A `.tape` exists for every gallery asset advertised in `README.md` (recreated `demo_fire` / `demo_spreadsheet`, added `demo`, `demo_dashboard`, `demo_website`, `demo_game`, `demo_pretext`), and `tests/gallery_manifest.rs` (plain `cargo test`, no VHS) fails if README ↔ tape parity drifts. The `pty-test` feature is exercised in the gate (clippy + `cargo test` default and `--features pty-test`, plus `cargo hack --each-feature` with `pty-test` isolated). Replaces the manual "tmux verification for visual demos before release" release-checklist line in `AGENTS.md`.

### Docs

- **`docs` — reconcile `API_DESIGN.md` / `NAMING.md` with the shipped surface** (#271) — Five doc-vs-code drifts resolved so the canonical design docs compile against the actual crate: `alert` arg order corrected to `(message, level)`; Rule 1 rewritten to the shipped consuming-builder shape (`mut self -> Self`, Drop-render, `.show()`) with the PR checklist updated to match; the `progress` doc corrected to the shipped `-> Response`; the `_colored` contradiction resolved in one direction by **blessing** the suffix as the canonical color-override variant (the `API_DESIGN.md` checklist now permits `_colored` and cross-refs `NAMING.md`); and the `NAMING.md` suffix table extended with `_hd` / `_halfblock` / `_styled` plus a v0.21.0 Deprecations section for the `code_block_*` aliases. The accompanying `code_block` builder + deprecations are listed under Changed (Breaking) and Deprecated.
